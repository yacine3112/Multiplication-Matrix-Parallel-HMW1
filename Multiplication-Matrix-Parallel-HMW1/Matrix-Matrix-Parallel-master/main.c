...
int main(int argc, char **argv){
	...
	/*Argument pour passage des paramtres*/
	blocksize Argument;
	Argument.NbrThreads = NBRTHREAD ;
	Argument.n = n;
	Argument.taille = n/NBRTHREAD;
	Argument.reste = n%NBRTHREAD;
	Argument.istart = 0;
	Argument.iend = n-1;
	Argument.A = A;
	Argument.B = B;
	...
	if((!strcmp(Mode,"S"))||(!strcmp(Mode,"s"))){
		...
		MultMatrices(A,B,n);
		fin = clock();
		temps = ((double)fin - debut) / CLOCKS_PER_SEC; //Calcul le temps d'execution pour le mode sequentiel
		...
		for(i=0; i<NBRTHREAD; i++){ 
			Argument.istart = i * Argument.taille;
			Argument.iend = Argument.istart + Argument.taille-1 + Argemunt.reste;
			debut = clock();
		...
		printMatrix(Argument.C,Argument.n);
		fin = clock();
